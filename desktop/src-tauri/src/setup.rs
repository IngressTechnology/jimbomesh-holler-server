...
pub async fn launch_services(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    *state.port.lock().unwrap() = port;
    ...
    match saved.as_ref().map(|c| c.mode.as_str()) {
        ...
        Some("standalone") if saved.as_ref().is_some_and(|c| c.setup_complete) => {
            if port_occupied {
                eprintln!("[holler-desktop] Setup complete, server already running \u{2192} opening Admin UI");
                do_attach(app, port, &env).await
            } else {
                eprintln!("[holler-desktop] Setup complete, starting services (no wizard)");
                do_standalone_resume(app, port).await
            }
        }
        Some("standalone") => {
...
/// Fast-path for subsequent launches after setup is already complete.
/// Starts Ollama + Holler server without the setup wizard UI.
async fn do_standalone_resume(app: &tauri::AppHandle, port: u16) -> Result<(), String> {
    process::cleanup_old_bundles(app);

    // Verify the server bundle still exists
    let _server_dir = match process::find_holler_server_dir(app) {
        Some(dir) => dir,
        None => {
            eprintln!("[holler-desktop] Server bundle missing — falling back to full setup");
            return do_standalone(app, port).await;
        }
    };

    // Ensure Ollama is running
    if !process::is_ollama_running().await {
        eprintln!("[holler-desktop] Starting Ollama...");
        if let Err(e) = process::start_ollama(app).await {
            eprintln!("[holler-desktop] Ollama start failed ({e}), falling back to full setup");
            return do_standalone(app, port).await;
        }
    }

    // Start Holler server
    eprintln!("[holler-desktop] Starting Holler server on port {port}...");
    if let Err(e) = process::start_holler(app, port).await {
        eprintln!("[holler-desktop] Holler start failed ({e}), falling back to full setup");
        return do_standalone(app, port).await;
    }
...
fn save_mode(app: &tauri::AppHandle, mode: &str, port: u16) {
    ...
    let cfg = config::DesktopConfig {
        ...
        setup_complete: existing.as_ref().map(|c| c.setup_complete).unwrap_or(false),
    };
...
fn mark_setup_complete(app: &tauri::AppHandle, port: u16) {
    let cfg = config::DesktopConfig {
    ...
    if let Err(e) = config::save(app, &cfg) {
        eprintln!("[holler-desktop] Failed to save setup_complete: {e}");
    }
    eprintln!("[holler-desktop] Marked setup as complete");
}
...
fn ensure_env(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    let holler_name = default_holler_name();
...
JIMBOMESH_HOLLER_NAME={holler_name}
...
fn default_holler_name() -> String {
    let hostname = gethostname::gethostname()
        .to_string_lossy()
        .to_string();
    sanitize_holler_name(&format!("Holler-{hostname}"))
}

fn sanitize_holler_name(name: &str) -> String {
